...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak of 1 means that he has hit once and then missed. A streak of 0 means that he has missed after another miss.
...
The distribution of Kobe streak is right skewed. His longest streak is 4 hits in a row.

```{r}
ggplot(data=kobe_streak, aes(x=length)) + geom_histogram(binwidth = 1)

kobe_streak %>%
  summarise(median = median(length), mean = mean(length))
```
...
1.  Describe the distribution of streak lengths. What is the typical streak 
    ...
    The distribution is still right skewed. The typical streak is 0.88 for a independent shooter with a 45%, while Kobe typical streak is 0.76. The longest streak for the simulated shots is still 4 shots in a row.
...
1.  If you were to run the simulation of the independent shooter a second time, 
    ...
    The distribution would be the exactly the same with the exact same results because we set the seed therefore all simulations would give the same results
...
1.  How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    No Kobe distribution of streak length is not the same to the simulations. We can see that Kobe does not have hot hands because we see that Kobe has less 2 - 4 streak compared to the simulation which had only 45% hit rate.
    
```{r}
kobe_streak$player = 'Kobe'
sim_streak$player = 'Sim'

ggplot(data = rbind(kobe_streak, sim_streak), aes(x = length, fill=player)) + geom_density(alpha = 0.2)
```
