...
```{r load-abrbuthnot-data, eval=FALSE, include=FALSE}
...
1.  What years are included in this data set? What are the dimensions of the 
    ...
    The years that are in this dataset is from 1940 to 2002. The dimensions of the dataframe is 63 rows and 3 columns. The column names are year, boys, girls
...
The counts in the present dataset is a lot larger compared to the Arbuthnor dataset
...
```{r}
ggplot() + geom_line(data = present, aes(x = year, y = boy_ratio), color = 'blue') + geom_line(data = arbuthnot, aes(x = year, y = boy_ratio), color = 'red')
```

We can see that arbuthnot observation observation does hold up in the US because we can see that a majority of the male ratio every year is over 50%
