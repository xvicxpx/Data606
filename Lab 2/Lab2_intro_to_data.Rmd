...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_dd = median(arr_delay), iqr_dd = IQR(dep_delay), n_flights = n())
```
...
July had the highest average departure delay. 
...
Would fly out of LGA as it has a 72% on time percentage
...
```{r}
ggplot(dl_aa_ua, aes(dep_delay, arr_delay, color = carrier)) + xlim(-20, 100) + ylim(-20, 100) + geom_point()
```
