...
> I wounld not expect the sample proportion to match another students as this is a random sample of 50 people from the population. I would expect the proportion to be very similar to what I got and also the population.
...
> We can see that sample 2 is a lot farther away from sample 1 and the population. The sample size of 1000 is more accurate because the larger the sample the closer it becomes to the population.
...
> There are 15,000 elements in sample_props50. The distribution is a normal distribution with its center at 0.2 which is what the population mean is.
...
> Each observation represents a simulation of taking a sample of 10 people. 
...
> we can see that with increase sample size the distribution becomes more normal and the SE also decereases.With the increase in simulations the SE does not change
...
> we can see that the sample that we took is a bad estiamate of the population
...
ggplot(data=sample_props15, aes(x=p_hat)) + geom_histogram(binwidth = 0.02) + labs(
    x = "p_hat (benefit)",
    title = "Sampling distribution of p_hat",
...
> The plot seems to be a approximately normal and skewed to the left. Based on the sampling distribution I would guess that the true proportion would be 0.8
...
> As we can see from the population that P for Benefits is exactly 0.8
...
ggplot(data=sample_props150, aes(x=p_hat)) + geom_histogram(binwidth = 0.02) + labs(
    x = "p_hat (benefit)",
    title = "Sampling distribution of p_hat",
    subtitle = "Sample size = 15, Number of samples = 2000"
  )
...
> We can see that the distribution with 150 samples is more normal compared to the distrbution that has the sample size of 15. Based on the sample distribution I would guess that the true proportion of people that think scientists do enhance their lives is 0.8
...
> We can see that the sampling distribution with a larger sample size had a smaller spread. If i needed to estimate the true value i would prefer a smaller spread as this means there is less variablilty on guessing what the true value is.
